...
export const MilestonesDropdown: NextPage<Props> = ({ handleMonthDropDown, activeMonthToShow }) => {
                ...
                <>
                    ...
                    <div className="mt-1 relative">
                        <Listbox.Button className="relative w-min bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 sm:text-sm">
                            {selected ? (
                                <span className="block truncate">{selected.month}</span>
                            ) : null}
                            ...
                            <Listbox.Options className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                {months.map((mth) => (
                                    <Listbox.Option
                                        ...
                                        value={mth}
                                        onClick={() => handleMonthDropDown(mth.objKeyName)}
                                    >
                                        {({ selected, active }) => (
                                            <>
                                                <span
